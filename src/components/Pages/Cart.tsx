...
import {setClear} from "../../redux/Cart/slice";

export const Cart: React.FC = () => {
	const dispatch = useDispatch();
	const {items, totalPrice} = useSelector(cartSelector);
	const count = items.reduce((acc: number, obj: any) => acc + obj.count, 0);
	const onClickRemove = () => {
		const answer = window.confirm('Вы уверены, что хотите очистить корзину?');
		answer && dispatch(setClear());
	}
				...
				<div className="cart__bottom">
					...
					<div className="cart__bottom-buttons">
						...
						<div className="button pay-btn">
							<Link to='/payment'
										onClick={onClickPayment}><span>Оплатить сейчас</span></Link>
						</div>
					</div>
				</div>
			</div>
